...
### Backend Setup
1. Create root project directory `spendsense/`
2. Create `backend/` directory
3. Initialize Python virtual environment in backend/
4. Create `backend/requirements.txt` with initial dependencies:
   ...
   - pydantic==2.5.0
   - python-dotenv==1.0.0
5. Create `backend/.env.example` file with placeholder environment variables
6. Create `backend/.gitignore` for Python projects
7. Install dependencies: `pip install -r requirements.txt`

### Backend Project Structure
8. Create `backend/app/` directory
9. Create `backend/app/__init__.py`
10. Create `backend/app/main.py` (empty FastAPI app skeleton)
11. Create `backend/app/database.py` (SQLAlchemy setup placeholder)
12. Create `backend/app/models.py` (empty models file)
13. Create `backend/app/schemas.py` (empty schemas file)
14. Create `backend/app/routers/` directory with `__init__.py`
15. Create `backend/app/services/` directory with `__init__.py`
16. Create `backend/app/utils/` directory with `__init__.py`
17. Create `backend/tests/` directory with `__init__.py`
...
27. Create `frontend/src/components/` directory
28. Create `frontend/src/lib/` directory
29. Create `frontend/src/lib/api.js` (Axios instance with base URL)
30. Update `frontend/.gitignore` to exclude node_modules and build files

### Data Directory
31. Create `data/` directory at root level
32. Create `data/.gitkeep` to track empty directory

### Scripts Directory
33. Create `scripts/` directory at root level
34. Create `scripts/__init__.py`

### Documentation
35. Create root `README.md` with project title and basic description
36. Create `docs/` directory
37. Create `docs/DECISIONS.md` placeholder
38. Create `docs/LIMITATIONS.md` placeholder

### Git Repository
39. Initialize git repository: `git init`
40. Create root `.gitignore` combining Python and Node patterns
41. Create initial commit with project structure
...
### Dependencies
1. Add `faker==20.1.0` to `backend/requirements.txt`
2. Install faker: `pip install faker`

### Script Creation
3. Create `scripts/generate_synthetic_data.py`
4. Import required libraries (Faker, json, random, datetime, uuid)
5. Set random seed for reproducibility: `random.seed(42)`

### User Generation Function
6. Create function `generate_users(count=75)` that returns list of user dicts
7. Generate 71 customers (95%) and 4 operators (5%)
8. For each user, generate:
...
9. Return list of user dictionaries

### Account Generation Function
10. Create function `generate_accounts(users)` that returns list of account dicts
11. For each user, generate 2-4 accounts with varied types:
...
12. For each account generate:
    ...
    - created_at
13. Return list of account dictionaries

### Transaction Generation Function
14. Create function `generate_transactions(users, accounts)` that returns list of transaction dicts
15. Define merchant categories with sample merchants:
...
16. For each user, generate 150-300 transactions over 180-day window
17. Ensure transaction patterns align with intended personas:
...
18. For each transaction generate:
    ...
    - created_at
19. Return list of transaction dictionaries

### Liability Generation Function
20. Create function `generate_liabilities(users, accounts)` that returns list of liability dicts
21. For each credit card account, generate liability record:
    ...
    - created_at
22. Ensure variety in credit behaviors:
    - 20% of users: high utilization (>50%)
    - 30% of users: medium utilization (30-50%)
    - 50% of users: low utilization (<30%)
23. Return list of liability dictionaries

### Main Execution Function
24. Create `main()` function that orchestrates data generation
25. Generate 75 users
26. Generate accounts for all users
27. Generate transactions for all users/accounts
28. Generate liabilities for credit card accounts
...
30. Write data to JSON files:
    - `data/synthetic_users.json`
    - `data/synthetic_accounts.json`
    - `data/synthetic_transactions.json`
    - `data/synthetic_liabilities.json`
31. Use pretty print (indent=2) for readability

### Testing & Validation
32. Add `if __name__ == "__main__":` block to call main()
33. Run script: `python scripts/generate_synthetic_data.py`
34. Verify all 4 JSON files created in data/ directory
35. Verify file sizes (transactions.json should be largest, ~5-10MB)
36. Spot-check JSON files for data quality:
    - Valid dates
    - Realistic amounts
    - Proper ID formats
    - Correct consent distribution (30% True)
37. Verify persona signal distribution in data:
...
38. Document any data generation assumptions in comments
